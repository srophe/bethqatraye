...
let $title := ($node/descendant::*[@srophe:tags="#headword"][1],$node/descendant::*[@syriaca-tags="#syriaca-headword"][1],$node/descendant::tei:title[1])[1] 
...
let $type := 
            if($id = concat($config:base-uri,'/place/37') or $id = concat($config:base-uri,'/place/37/tei')) then 
                'bq'
            else if($node//tei:region[@ref= concat($config:base-uri,'/place/37')] and $node//tei:location[@type="gps"][@subtype="representative"]) then 
                'bqRepresentative' 
             else if($node//tei:location[@type="gps"][@subtype="representative"]) then 
                'representative' 
             else if($node//tei:region[@ref= concat($config:base-uri,'/place/37')]) then 
                'bqRegion' 
             else ()   
let $coords := if($node//tei:location[@subtype = 'preferred']) then $node//tei:location[@subtype = 'preferred']/tei:geo else $node//tei:geo[1]
return 
    <json:value>
